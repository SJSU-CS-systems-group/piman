"""
Used ntp server code from https://github.com/limifly/ntpserver
"""
...
class RecvThread(threading.Thread):
    def __init__(self, ssocket):
        ...
        self.ntpsocket = ssocket

    def run(self):
        global taskQueue, stopFlag
        while True:
            if stopFlag == True:
                print("NTP RecvThread Ended")
                break
            rlist, wlist, elist = select.select([self.ntpsocket], [], [], 1)
            if len(rlist) != 0:
                print ("NTP Received %d packets" % len(rlist))
    ...
    def run(self):
        ...
        while True:
            ...
            try:
                ...
                #addr[0] = "172.30.6.12"
                #addr[1] = 3333
                self.ntpsocket.sendto(sendPacket.to_data(), addr)
                print("NTP Sended to %s:%d" % (addr[0], addr[1]))
            except queue.Empty:
                continue
...
def do_ntp():
    ...
    ntpsocket.bind((listenIp, listenPort))
    print("local socket: ", ntpsocket.getsockname())
